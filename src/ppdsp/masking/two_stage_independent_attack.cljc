...
(defn remove-independent-noise-attack
  [io-attack-data rng ] ;window-no
  (let [best-noise-attack (get-best-independent-noise io-attack-data rng ) ;window-no
...
(defn reverse-rp
  [independent-noise? bias-input-prob? io-attack-data rng ] ;window-no
  (->>
   (for [_ (range 1)]
...
(defn known-io-projection-and-independent-noise-map-attack-two-stage
  ...
  (let [independent-noise? (if (nil? independent-noise?)
        ...
        [rp-io-attack-data best-noise-attack] (if independent-noise?
                                                (remove-independent-noise-attack io-attack-data rng ) ;window-no
                                                [io-attack-data nil])
        masked-feature-count (attack-masked-feature-count io-attack-data)
        record-count (count (:knowns io-attack-data))
        rp-result (reverse-rp independent-noise? bias-input-prob? rp-io-attack-data rng )] ;window-no
    (assoc rp-result
