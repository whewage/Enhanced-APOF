...
(defn get-noise-difference-sigmas
  ...
  [io-attack-data]
  (let [cumulative-variance (expt (:cumulative-noise-sigma io-attack-data) 2)
    ...
    (->>
     (for [_ (range 1)]
       (let [initial-variables (generate-initial-noise-differences! io-attack-data rng)
         ...
         (nelder-mead-optimize objective-fn initial-variables true ))) ;window-no
     (apply max-key :score))
    )
 )
...
(defn remove-cumulative-noise-attack
  [io-attack-data rng ] ;window-no
  (let [best-noise-attack (get-best-cumulative-noise io-attack-data rng ) ;window-no
...
(defn reverse-rp
  [cumulative-noise? bias-input-prob? io-attack-data rng ] ;window-no
  (->>
   (for [_ (range 1)]
...
(defn known-io-projection-and-cumulative-noise-map-attack-two-stage
  ...
  MA (pages 371-373)."
  [io-attack-data rng ;window-no
   & {:keys [optimization-max-evaluations optimization-relative-threshold ;window-no
  ...
  (let [cumulative-noise? (if (nil? cumulative-noise?)
                            ...
                            (> (:cumulative-noise-sigma io-attack-data) 0)
                            cumulative-noise?
                          )
        ...
        [rp-io-attack-data best-noise-attack] (if cumulative-noise?
                                                (remove-cumulative-noise-attack io-attack-data rng ) ;window-no
                                                [io-attack-data nil])
        masked-feature-count (attack-masked-feature-count io-attack-data)
        record-count (count (:knowns io-attack-data))
        rp-result (reverse-rp cumulative-noise? bias-input-prob? rp-io-attack-data rng )] ;window-no
    (assoc rp-result
