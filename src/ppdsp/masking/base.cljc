                                    ...
                                    (map-indexed
                                     (fn [idx masked-group-schema]
                                       (map (fn [feature-schema]
                                              (update feature-schema :name #(str idx "_" %)))
                                            ;;(update feature-schema :name #(str %)))
                                            masked-group-schema)))
                                    (reduce concat))
        ;; Combine the masked features schema with the original class
        ;; schema
        masked-schema (conj (vec masked-schema-features) (last schema))]
    ;;(println schema-features "features")  ;;{:name avg_rss12, :options :numeric} ...
  ;;  (println feature-keys "keys") ;;(avg_rss12 var_rss12 avg_rss13 var_rss13 avg_rss23 var_rss23)
  ;;  (println masked-schema-features "masked-schema-featues")  ;;{:name 0-dim0, :options :numeric} {:name 0-dim1, :options :numeric} {:name 0-dim2, :options :numeric} {:name 0-dim3, :options :numeric} {:name 0-dim4, :options :numeric} {:name 0-dim5, :options :numeric}
   ;; (println masked-schema "masked-schema")
   ;;(println feature-group-masks)
