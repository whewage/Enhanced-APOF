...
(defn make-random-projection-with-noise-and-translation-mask-factory-cycles 
  ...
  ;;Random cycles
  (def cycles [(* 1 cycle-size) (* 2 cycle-size) (* 4 cycle-size) (* 5 cycle-size) (* 6 cycle-size)]) ;;Half of the cycle size;; Full- 1000 2000 4000 5000 6000
  (def cs (rand-nth cycles))
  
   (def counter (atom 0))
  (def logistic-counter (atom (- (* -1 cs) 1 )) ) ;;start logistic counter with - values
  ;;(def logistic-counter (atom (- lower-bound 1 ))) ;;this cannot be used when selecting random cycle sizes
                        ...
                        identity
		                        (fn [features]
		                           (when features (swap! logistic-counter inc)   
                                     (if (> @logistic-counter cs)         ;;(if (> @logistic-counter upper-bound)
                                        (do                   ;;(reset! logistic-counter   (- lower-bound 1 ) )    ;;resetting the cycles from lower bound to upper bound
                                           (def cs (rand-nth cycles)) ;;select a random cycle size at the end of the current cycle
                                          (reset! logistic-counter   (- (* -1 cs) 1 ) )
 ...
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;        
          ...
          ;;Try calling loss function here
          
          
          ;;;;;;;;;;;;;;;;;;;
          ;LC (println "end LC" @logistic-counter)
         ; CS (println "end CS" cs)
          ]
