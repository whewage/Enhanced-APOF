  ...
  (:require [clojure.string :as string]
            ...
            [ppdsp.masking.evaluation
             :refer [unknown-record-displacement
                    ...
                    getting-original-model-description
                    unknown-record-relative-position add-combined-result flatten-masking-experiment-recoveries
                     ]]
            [ppdsp.classifier.base
             :refer [process-record describe-model]] ;;waruni
            [ppdsp.classifier.moa-classifier
             :refer [hoeffding-tree]] ;;waruni
            [ppdsp.trees.parse-model
             :refer [parse-moa-tree-model-string]] ;;waruni
            [ppdsp.dataset.base :refer [get-schema dataset-record-count]];;waruni
           ...
           ;; [semantic-csv.impl.core :as impl :refer [?>>]] ;;waruni
            ...
            [incanter core charts pdf]
            )
  
  (:import [ppdsp.classifier.base Classifier]
           [com.twosigma.beakerx.table TableDisplay]
...
#_(defn display-masking-error-plots
...
(defn accuracy-time-plot
  [results-cumulative-logistic classifier  growth-rate-k  ;;results-rp-only     results-independent results-cumulative results-cumulative-cycles
...
#_(defn attack-strategy-comparison-plots
...
#_(defn accuracy-privacy-tradeoff-legend
...
#_(defn accuracy-privacy-tradeoff-comparison
...
#_(defn accuracy-privacy-tradeoff
   [results-cumulative results-cumulative-cycles results-cumulative-logistic ;;results-rp-only   ;;results-independent
    classifier cumulative-attack-strategy cumulative-cycles-attack-strategy logistic-attack-strategy                         ;;independent-attack-strategy
...
#_(defn attack-strategy-comparison
  ...
  (letfn [(round-up [krc]
    ...
    (->> results
         (map
          (fn [result]
            (-> result
                (update-in [:privacy-attacks :configuration :known-record-counts] ;;waruni-changed
                           #(map round-up %))
                (update-in [:privacy-attacks :evaluations] ;;waruni changed
                           (fn [evaluations]
                             (map #(update % :known-record-count round-up)
                                  evaluations)))))))))

 ;;Measuring the breach probability after performing random attacks to the data stream
#_(defn attack-whole-stream [results temp-bp-file epsilon attack-count ]
 (def counterX (atom 0))
            (for [result results]
			       	(if  (= (-> result :strategy):a-rpcn) ;;consider the privacy for the attack strategy a-rpcn
                           ...
                           (save-data temp-bp-file (double (/ count-value attack-count)) )
		                        )
				   )
			  )
 ...
 ;;Measuring the breach probability after performing random attacks to each window
(defn attacks-per-window [results bp-window-file epsilon attack-count record-count window-size] ;temp-bp-file
 (def counterX (atom 0))
 (def no-of-windows (int (ceil (/ record-count window-size ))))
 ;(println results)
         (loop [i 0]
            (when (<= i (- no-of-windows 1))       
		           (dorun (for [result results]
		               (if (and (= (-> result :strategy):a-rpcn) (and (= (-> result :window-no) i) (<= (-> result :relative-error) epsilon)))
                 ...
                 (save-data-append-newline bp-window-file (double (/ @counterX (/ attack-count no-of-windows))))
                 (reset! counterX 0)
             (recur (+ i 1))  
          )
...
;;Record average error for the data stream - Needs to check. gives null pointer exception for cnt-value
#_(defn record-average-accuracy [dataset original-acc-results masked-acc-results final-accuracy-file growth-rate-k acc]
   (with-open [writer (io/writer final-accuracy-file :append true)]
     (let [ counter (atom 0)
			       record-count  (dataset-record-count dataset)
			       cnt-value (loop [i 1]
													(when (<= i record-count)
											       (if (=(get-in masked-acc-results [i :class]) (get-in masked-acc-results [i :truth]))
													      (swap! counter inc)
													         (+ @counter 0)    
											         )
			                      (recur (+ i 1))
        ...
        ;  x (println cnt-value)
          acc-value (double (/ cnt-value record-count))
          final-acc(for [i (range 1)]
					                            [growth-rate-k acc-value (- 1 acc-value)]
					                       )
        ]
        (csv/write-csv writer final-acc)   
	    )
    )
)
;;;;;;;;;;;;;;;;;;;

;;Calculatae and record avarage accuracy for each window(waruni)
(defn accuracy-updating-windows [results win-acc-excel-file record-count win-acc-details-file average-error-win-file window-size]
 (def counter (atom 0))
 (def x (atom 0))
 (def details-vec (atom {:sample-accuracy {}}))
  (loop [i 1]
    (when (<= i record-count)
					    (if (= i (+ 1 (* window-size (+ @x 1)))) ;;to get the relative id according to the cycle
						     (do
                   (save-data-append-newline average-error-win-file (- 1 (double (/ @counter window-size))))
					         (swap! x inc)
					         (reset! counter 0)
		    ...
		    (let[ cnt-value (if (=(get-in results [i :class]) (get-in results [i :truth]))
		                      ...
		                      acc-value  (/ cnt-value (- i (* window-size @x))) ;;divided by the relative id for the cycle
                ...
                (swap! details-vec assoc-in [:sample-accuracy  i] {:id i :value double-value}) ;;appending values return from each iteration to a atom vector
		                  (save-data-append-newline win-acc-excel-file double-value) ;;for excel operations    
		     )
      (recur (+ i 1))
    )
  )
  (save-data-append-newline average-error-win-file (- 1 (double (/ @counter (- record-count (* window-size @x)))))) ;;write the error of last window.
	(save-data win-acc-details-file @details-vec) 
)

;;Read the results written to the files and proccessing accuracy-privacy results to calculate error and breach probability. 
(defn save-bp-error-final [ growth-rate-k output-file attack-count cumulative-noise-sigmas epsilon  ;dataset  breach-probability-file 
                           win-acc-details-file  win-acc-excel-file record-count average-error-win-file window-size bp-window-file] ;temp-bp-file final-accuracy-file 
; (with-open [writer (io/writer breach-probability-file :append true)]
    (let [ output-cumulative-logistic (load-data output-file)
         ;temp-bp-file temp-bp-file
           results-cumulative-logistic (-> (filter #(contains? (set cumulative-noise-sigmas) (:cumulative-noise-sigma %)) ;;All results without original accuracy (masking results)
                                  (:results output-cumulative-logistic))
                                     (add-combined-result :score [:a-rp :a-rpcn])
                                     (add-combined-result :score [:a-rp :a-rpcn-1])
                                     )
         
           flat-results-cumulative-logistic (->> results-cumulative-logistic flatten-masking-experiment-recoveries
                                         (map #(assoc % :unknown-record-relative-position (unknown-record-relative-position %)))  ;;results with privacy
                                 )
         ;;  attacks-cycle (doall(attack-whole-stream flat-results-cumulative-logistic temp-bp-file epsilon attack-count )) ;;Attack to stream and record bp
         ;;bp calculation to whole stream is not suitable as attacks are performed to windows separately. Therefore bp per window was calculated
          bp-per-window (doall(attacks-per-window flat-results-cumulative-logistic bp-window-file epsilon attack-count record-count window-size));temp-bp-file
         
   ;;;;;;;;;;;;;;;;To maintain the fairness accuracy should be calculated for the whole stream.  
         original-accuracy-cumulative-logistic (-> output-cumulative-logistic :original :accuracy) ;;original classification results with accuracy
         access-result-list-logistic  (nth results-cumulative-logistic 0) ;;nth is used to access elements of lists
         acc-after-masking-logistic  (-> access-result-list-logistic :accuracy :hdt :raw-results );;getting only accuracy results after masking
         acc (double (-> access-result-list-logistic :accuracy :hdt :accuracy ))
      ;   error-calculation (record-average-accuracy dataset original-accuracy-cumulative-logistic acc-after-masking-logistic final-accuracy-file growth-rate-k acc)
   
      ;;;;;;;;;;;;;;Window-based accuracy calculation is implemeted because it is more suitable for infinite data streams
        win-acc (accuracy-updating-windows acc-after-masking-logistic win-acc-excel-file record-count win-acc-details-file 
                                           average-error-win-file window-size)
   ;;;;;;;;;;;;;;;;;;;;
     
				;final-bp (load-data temp-bp-file)
					          
				;final-breach (for [i (range 1)]
					                           ;[growth-rate-k final-bp (- 1 acc) ] 
					                     ;)
        
          ]
  (println "Breach probability and accuracy was calculated")
     ; (csv/write-csv writer final-breach)         
	;)
  )
)
								  
(defn run-masking-experiments-cycles
  [{:keys [dataset  output-file output-file-privacy-auc output-file-privacy-attacks 
           projection-feature-counts projection-sigmas 
          original-model-description original-probability-file masked-model-file masked-probability-file  
           anytime-ael-file bp-window-file ;final-accuracy-file temp-bp-file final-bp-file
          cumulative-noise-sigmas translations cycle-sizes epsilon
          attack-count attempt-count known-record-counts known-record-ranges
           known-record-range-position attack-strategies
            growth-rate-k-initial maximum-fn-value previous-upper-bound-file-to-update flat-record-length
            threads-per-configuration  record-count  window-size previous-AUC-file-to-update    ;; cycle-size-upper-bound pre-AUC bandwidth
          classifier-fns threads-per-evaluation seed evaluations 
          win-acc-details-file win-acc-excel-file average-error-win-file
          ]}]
 (getting-original-model-description dataset original-model-description original-probability-file)
 (loop [growth-rate-k growth-rate-k-initial]
   (when (<= growth-rate-k 0.1)  ;;(<= growth-rate-k 0.1)
		  (time
		   (let [configurations (for [projection-feature-count projection-feature-counts
		                              ...
		                              ;;cycle-size-upper-bound cycle-size-upper-bound
		         ...
		         results (pmap-pool threads-per-configuration
		                            (fn [{:keys [projection-feature-count cumulative-noise-sigma 
		                              translation projection-sigma cycle-size]}] ;
		                                 ...
		                                 :pre-auc-file previous-AUC-file-to-update 
		                                 :pre-upper-file previous-upper-bound-file-to-update
		                                 :masked-model-file masked-model-file
		                                 :masked-probability-file masked-probability-file
                                     :anytime-ael-file anytime-ael-file
		                                 :output-file-privacy-auc output-file-privacy-auc 
		         ...
		         output {:original {:accuracy (map-vals #(test-classification-accuracy % dataset) ;;Original accuracy of the data set
		                                     classifier-fns )}
		                :results results}
		             ]
            (println "Growth Rate = " growth-rate-k)
		     (save-data output-file output)
            (println "Attacks were performed")
		         (doall (save-bp-error-final growth-rate-k output-file attack-count cumulative-noise-sigmas epsilon
                                       win-acc-details-file  win-acc-excel-file record-count average-error-win-file window-size bp-window-file)) ;  final-bp-file  dataset temp-bp-file final-accuracy-file
          (println "Written into the file")
		         
		     nil)  
		       ) 
     (recur (Double/parseDouble(format "%.3f" (+ growth-rate-k 0.004)))) ;;
   )
    )
)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn validate-datafitting-results
  [{:keys [dataset  output-file output-file-privacy-auc output-file-privacy-attacks 
           projection-feature-counts projection-sigmas 
          original-model-description original-probability-file masked-model-file masked-probability-file  
          anytime-ael-file final-bp-file temp-bp-file ;final-accuracy-file
          cumulative-noise-sigmas translations cycle-sizes epsilon
          attack-count attempt-count known-record-counts known-record-ranges
           known-record-range-position attack-strategies
            growth-rate-k-initial maximum-fn-value previous-upper-bound-file-to-update flat-record-length
            threads-per-configuration  previous-AUC-file-to-update   
          classifier-fns threads-per-evaluation seed evaluations 
          ]}]
 (getting-original-model-description dataset original-model-description original-probability-file)
 (for [growth-rate-k growth-rate-k-initial ] 
		  (time
		   (let [configurations (for [projection-feature-count projection-feature-counts
		                              cumulative-noise-sigma cumulative-noise-sigmas
		                              translation translations
		         ...
		         results (pmap-pool threads-per-configuration
		                            (fn [{:keys [projection-feature-count cumulative-noise-sigma 
		                              translation cycle-size projection-sigma ]}]
			                              (masking-experiment-cycles    
			                               :raw-dataset dataset
			                               :projection-features projection-feature-count
			                               :projection-sigma projection-sigma
            ...
            (println "Growth Rate = " growth-rate-k)
		     (save-data output-file output)
            (println "Attacks were performed")
		         (doall (save-bp-error-final growth-rate-k output-file attack-count cumulative-noise-sigmas epsilon final-bp-file temp-bp-file  )) ;dataset final-accuracy-file
          (println "Written into the file")
...
#_(defn accuracy-updating-cycles [results accuracy-cycles-outputfile record-count full-cycle details-outputfile]
  (def counter (atom 0))
  (def x (atom 0))
  (def details-vec (atom {:sample-accuracy {}}))
   (loop [i 1]
...
#_(defn accuracy-for-flat-areas [accuracy-details full-cycle flat-record-length record-count accuracy-period-file]
  (def acc-sum (atom 0)) 
